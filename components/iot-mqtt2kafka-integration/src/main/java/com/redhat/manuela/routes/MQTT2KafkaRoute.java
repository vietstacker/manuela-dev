...
public class MQTT2KafkaRoute extends RouteBuilder {
	...
	private void storeTemperatureInKafka() {
		from("paho:{{mqtt.broker.topic.temperature}}?brokerUrl={{mqtt.broker.uri}}&clientId={{mqtt.broker.clientId}}")
			.log("Reading message form MQTT: ${body}")
			// .setHeader(KafkaConstants.KEY, constant("sensor-temp"))
			.to("kafka:{{kafka.broker.topic.temperature}}?brokers={{kafka.broker.uri}}&clientId={{mqtt.broker.clientId}}")
			.log("sent message: ${headers[org.apache.kafka.clients.producer.RecordMetadata]}");
	}

	private void storeVibrationInKafka() {
		from("paho:{{mqtt.broker.topic.vibration}}?brokerUrl={{mqtt.broker.uri}}&clientId={{mqtt.broker.clientId}}")
			.log("Reading message form MQTT: ${body}")
			// .setHeader(KafkaConstants.KEY, constant("sensor-temp"))
